...
class ClassExpression extends Expression
{
    ...
    public function toTypeScript(ExpressionStringGenerationOptions $options, GenerationContext $context): string
    {
        ...
        return $this->toTypeScriptClass($options, $context);
    }
    ...
    private function toTypeScriptEnum(ExpressionStringGenerationOptions $options): string
    {
        ...
        // Generate service object for enums with service methods
        if ($this instanceof EnumExpression && !empty($this->serviceMethods)) {
            $content .= "\n" . $this->generateServiceObject($options);
        }
    ...
    private function toTypeScriptClass(ExpressionStringGenerationOptions $options, GenerationContext $context): string
    {
        // Collect all required imports from members using context
        $this->collectRequiredImports($context);

        $extends = $this->extends ? " extends " . $this->extends : '';
        $content = null;
        foreach (Arr::sort($this->imports, fn (ImportExpression $import) => $import->isInternal) as $import) {
            $content .= $import->toTypeScript($options, $context) . "\n";
        }
        ...
        foreach ($this->members as $member) {
            if (!$member->noConvert && $member->canGenerateInBody($options)) {
                $memberTypescript = $member->toTypeScript($options, $context);
                if (!$memberTypescript) {
                    continue;
                }
    ...
    /**
     * Collect required imports from all class members
     */
    private function collectRequiredImports(GenerationContext $context): void
    {
        // Start with existing imports
        $allImports = $this->imports;
